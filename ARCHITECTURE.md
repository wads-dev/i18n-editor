...
Review baselines are IndexedDB-persisted and project-scoped because they can contain large key sets. The selected **Last changes only** filter is a project-scoped `localStorage` preference. A baseline contains only the translation key set at the moment a user explicitly marks it reviewed. It is never created or updated by loading, editing or exporting a bundle. The filter compares the in-memory bundle with that baseline: added keys render normally, while removed keys render as read-only red rows. Removed rows are visible only while the filter is selected. Value-level review is a future extension.
